  ...
  return (
    <section className="py-16 md:py-32 px-4 bg-black/40 relative overflow-hidden">
      <div className="container mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 md:gap-12 max-w-6xl mx-auto relative">
          {/* Connecting Line */}
          ...
          {steps.map((step, index) => (
            <div
              key={index}
              className="relative flex flex-col items-center text-center space-y-3 sm:space-y-4 md:space-y-6 group"
            >
              {/* Icon Circle */}
              <div className="relative z-10 w-14 h-14 sm:w-16 sm:h-16 md:w-24 md:h-24 rounded-full bg-background border border-white/10 flex items-center justify-center group-hover:border-primary/50 transition-all duration-500 shadow-lg group-hover:shadow-primary/20">
                ...
                <step.icon className="w-6 h-6 sm:w-7 sm:h-7 md:w-10 md:h-10 text-muted-foreground group-hover:text-primary transition-colors duration-500" />

                {/* Number Badge */}
                <div className="absolute -top-2 -right-2 w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-primary flex items-center justify-center text-[10px] sm:text-xs md:text-sm font-bold text-white shadow-lg">
                  {step.number}
                </div>
              </div>
              ...
              <div className="space-y-2 md:space-y-3 px-4">
                <h3 className="text-base sm:text-lg md:text-2xl font-bold text-white group-hover:text-primary transition-colors duration-300">
                  {step.title}
                </h3>
